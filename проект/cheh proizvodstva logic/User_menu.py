...
class User:
    ...
    def user_mainmenu(self, a, b):
        ...
        mi = mi.subsample(a, b)
        self.aitems = Button(self.mainframe, text="Материалы", bd=5, font="roboto 11 bold", bg="cyan", image=mi, compound=TOP, command=self.builditemtable)
        self.aitems.image = mi
        self.aitems.place(x=260, y=17)
        mi = PhotoImage(file="images/invoice2.png")
        mi = mi.subsample(a, b)
        self.aitems = Button(self.mainframe, text="Заказ", bd=5, bg="cyan", font="roboto 11 bold", image=mi, compound=TOP, command=self.make_invoice)
        self.aitems.image = mi
        self.aitems.place(x=62, y=17)
        mi = PhotoImage(file="images/change1.png")
        mi = mi.subsample(a, b)
        self.changeuser = Button(self.mainframe, text="Смена ", bd=5, bg="cyan", font="roboto 11 bold", image=mi, compound=TOP)
        self.changeuser.image = mi
        self.changeuser.place(x=460, y=17)
        mi = PhotoImage(file="images/Door_Out-512.png")
        mi = mi.subsample(a, b)
        self.logout = Button(self.mainframe, text="Выйти", bd=5, bg="cyan", font="roboto 11 bold", image=mi, compound=TOP)
    ...
    def builditemtable(self):
        ...
        self.tree.heading('Stocks', text="Осталось", anchor=W)
    ...
    def make_invoice(self):
        ...
        self.tree = ttk.Treeview(self.tableframe1, columns=("Transaction ID", "Product ID", "Product Name",
                                                            'Quantity', 'Price', 'Date', 'Time'), selectmode="browse", height=6,
                                 yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
        self.tree.column('#0', stretch=NO, minwidth=0, width=0)
        ...
        self.tree.heading('Transaction ID', text="id оплаты", anchor=W)
        self.tree.heading('Product ID', text="id продукта", anchor=W)
        self.tree.heading('Product Name', text="Название продукта", anchor=W)
    ...
    def addtotrans(self):
        if (len(self.additem.get()) == 0 or self.inventory.count(self.additem.get()) == 0):
            ...
            if ((li[0][0] - self.id_qty[row[0][1]]) - int(self.qty.get()) < 0):
                if (li[0][0] != 0):
                    messagebox.showerror('Ошибка', 'Материал с таким количеством недоступен!')
                else:
                    messagebox.showerror('Ошибка', 'Материал отсутствует в наличии!')
                return
    ...
    def removecart(self):
        ...
        if (len(re) == 0):
            messagebox.showerror('Ошибка', 'Не выбрана корзина')
            return
        if (messagebox.askyesno('Внимание!', 'Удалить корзину?') == True):
            x = self.tree.get_children()
            re = re[0]
            l = []
            fi = []
    ...
    def makeprint(self):
        if messagebox.askyesno("Внимание!", "Распечатать эту транзакцию?") == True:
            pass
        '''
            x = datetime.datetime.now()
            x = str(x.hour) + ' - ' + str(x.minute) + ' - ' + str(x.second)
